...
///VITO
//util.inherits(CategoryContentListView, ListView);
//function CategoryContentListView(viewModel) {
//  console.warn("DAFUQ - CategoryContentListView", viewModel);
//  this.htmlify = function(category) {
////    return '<li class="category-content arrow_box"><div class="content-wrapper">Content for ' + category.title + '</div></li>';
//    return '<li class="category-content arrow_box"><div class="content-wrapper"><ul class="content-list"><li>Loading ' + category.title + '</li></ul></div></li>';
//  };
//
//  this.identify = function(category) {
//    return category.id;
//  };
//
//  ListView.call(this, viewModel.categories(), viewModel.selectedCategories(), '#categorycontentlist', '#categorycontentwrapper', '#categorycontent');
//}
...
function ContentListView(viewModel) {
  console.warn("DAFUQ ContentListView", viewModel);
  this.htmlify = function(value) {
    console.warn("DAFUQ III", value);
    return '<li>Cthulhu Fhtagn!< /li>';
    //TODO define .media_item scss
//    return '<li class="media_item"><p>' + ((typeof value.item.type === 'string' && value.item.type.toLowerCase().indexOf("image") === 0) ? ('<img src="' + value.item.thumbnailURIs[0] + '">') : (value.item.title)) + '</p><img src="images/add.svg"></li>';
  };
...
//  var categoryContentListView = new CategoryContentListView(viewModel);
