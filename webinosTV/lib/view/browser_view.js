...
function DeviceListView(items, selection, list, wrapper, fadeout) {
  this.htmlify = function (device) {
    return '<li><img class="device-image" src="images/tv.svg"><div class="device-name">' + device.address() + '</div><div class="device-type">' + device.type().charAt(0).toUpperCase() + device.type().slice(1) + '</div></li>';
  };
